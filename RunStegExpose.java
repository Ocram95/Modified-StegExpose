...
public class RunStegExpose {
	...
	private static Long time_ps=null;
	private static Long time_cs=null;
	private static Long time_sp=null;
	private static Long time_rs=null;

	
	private static int algorithm_used=0;	
	/**
	...
	public static void main(String[] args){
		...
		if(csvMode){
			...
			writer.println();
			if(fast)
				writer.println("File name,Above stego threshold?,Secret message size in bytes (ignore for clean files),Primary Sets,Chi Square,Sample Pairs,RS analysis,Fusion (mean & fast), time_ps, time_cs, time_sp, time_rs, alg_used");
			else
				writer.println("File name,Above stego threshold?,Secret message size in bytes (ignore for clean files),Primary Sets,Chi Square,Sample Pairs,RS analysis,Fusion (mean), time_ps, time_cs, time_sp, time_rs, alg_used");
		...
		for (File file : listOfFiles) {
			...
			if (file.isFile()) {
		        ...
		        if(image != null){
		        	...
		        	long ps_startTime = System.nanoTime();
					try{
					...
					time_ps = System.nanoTime() - ps_startTime;
					...
					long sp_startTime = System.nanoTime();
					try{
					...
					time_sp = System.nanoTime() - sp_startTime;
					...
					long cs_startTime = System.nanoTime();
					try{
					...
					time_cs = System.nanoTime() - cs_startTime;
					...
					long rs_startTime = System.nanoTime();
					try{
					...
					time_rs = System.nanoTime() - rs_startTime;
					printResults2(4);
				}
	        }
		}
	...
	private static void printResults(){
		...
		if(csvMode){
			writer.println(fileName+","+stego+","+fusionQ+","+ps+","+cs+","+sp+","+rs+","+fusion+","+time_ps+","+time_cs+","+time_sp+","+time_rs+","+algorithm_used);
			writer.flush();
		}
		else
			if(stego)
				System.out.println(fileName + " is suspicious. Approximate amount of hidden data is "+fusionQ+" bytes." + algorithm_used);
	...
	private static boolean isClean2(int x){
		if(fast){
			if(Fuse.se(stegExposeInput)<threshold){
				printResults2(x);
				return true;
		...
		private static void printResults2(int x){
		//setting up stegexpose and quantitative stegexpose detector
		fusion = Fuse.se(stegExposeInput);
		fusionQ = Math.round(Fuse.seQ(fusion, fileSize));
		//determine is a file is a stego or clean file
		...
		if(csvMode){
			writer.println(fileName+","+stego+","+fusionQ+","+ps+","+cs+","+sp+","+rs+","+fusion+","+time_ps+","+time_cs+","+time_sp+","+time_rs+","+x);
			writer.flush();
		}
		else
			if(stego)
				System.out.println(fileName + " is suspicious. Approximate amount of hidden data is "+fusionQ+" bytes." + x);
